...
router.get('/chat', function (request, response) {
    ...
    var loginHtmlPath = path.join(__dirname, 'views', 'chat.html');

    // login    .html 파일 읽어오기
    fs.readFile('./views/login.html', 'utf8', function(err, body){
...
router.get('/login', function (request, response) {
    ...
    var loginHtmlPath = path.join(__dirname, 'views', 'login.html');
...
router.post('/login_process', function (request, response) {
    ...
    if (username && password) {             // id와 pw가 입력되었는지 확인
        db.query('SELECT * FROM usertable WHERE username = ? AND password = ?', 
...
router.get('/register', function(request, response) {
    ...
    // signup.html 파일의 경로 설정
    var loginHtmlPath = path.join(__dirname, 'views', 'signup.html');

    // signup.html 파일 읽어오기
    fs.readFile('./views/signup.html', 'utf8', function(err, body){
        var html = template.HTML(title, body, '');
        response.send(html);
    });
...
router.post('/register_process', function(request, response) {    
    ...
    if (username && password && password2) {
        db.query('SELECT * FROM usertable WHERE username = ?', 
        [username], function(error, results, fields) {          // DB에 같은 이름의 회원아이디가 있는지 확인
            if (error) throw error;
            if (results.length <= 0 && password == password2) { // DB에 같은 이름의 회원아이디가 없고, 비밀번호가 올바르게 입력된 경우 
                db.query('INSERT INTO usertable (username, password) VALUES(?,?)', 
                [username, password], function (error, data) {
